...
while true; do
    ...
    case $source in
        1)
            ...
            sed -i "s|https://hub.huhstsec.top/|$source_add|g" ./config-auto/agent/agent-temp.sh
            sed -i "s|https://hub.huhstsec.top/|$source_add|g" ./config-auto/k3s/registries.yaml
            break
            ;;
        2)
            echo "选择阿里云镜像服务..."
